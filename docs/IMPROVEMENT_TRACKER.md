...
## 八、前端功能改进记录 (2025-11-30)

### 8.1 已完成改进

#### (3) 计算器工具移至设置菜单 ✅
- **目的**: 强调系统端到端自动化能力，将独立工具降级为高级功能
- **修改文件**: `frontend/src/components/AppLayout.vue`
- **改动内容**:
  - 从主导航移除计算器入口
  - 在侧边栏底部添加"设置"下拉菜单
  - 计算器放入设置菜单，附描述"独立计算利息工具"

#### (1) 项目基本信息锁定标识 ✅
- **目的**: 防止用户误解可编辑，明确项目创建后信息不可变
- **修改文件**: `frontend/src/pages/ProjectDetailPage.vue`
- **改动内容**:
  - 在项目信息卡片右上角添加 🔒 已锁定 标识
  - Hover 时显示提示"项目创建后基本信息不可修改"

#### (4) 智能材料导入功能 ✅
- **目的**: 反转流程 - 从「手动输入债权人」变为「上传材料 → AI解析 → 自动生成」
- **新增文件**:
  | 文件 | 用途 |
  |------|------|
  | `frontend/src/components/MaterialUpload.vue` | 材料上传组件（拖拽/选择） |
  | `frontend/src/components/CreditorPreview.vue` | 解析结果预览/编辑 |
  | `frontend/src/components/SmartImportModal.vue` | 整合上传+预览流程 |
  | `app/api/parse_routes.py` | 后端解析 API |
- **修改文件**:
  | 文件 | 改动 |
  |------|------|
  | `frontend/src/components/CreditorList.vue` | 添加"智能导入"按钮 |
  | `frontend/src/api/client.ts` | 添加 `parseApi` |
  | `api_server.py` | 注册 parse_router |
  | `app/core/config.py` | 添加 ANTHROPIC_API_KEY |
- **功能流程**:
  1. 用户点击"智能导入"按钮
  2. 上传 PDF/图片材料（支持拖拽）
  3. Claude 分析材料，提取债权人信息
  4. 用户预览/编辑解析结果
  5. 确认后批量创建债权人
...
- **状态**: 后续实施
- **预计改动**:
  - `frontend/src/pages/NewProjectPage.vue` - 添加"从裁定书导入"选项
  - `app/api/parse_routes.py` - 添加 `/parse/ruling` 端点

### 8.3 环境配置注意事项

使用智能导入功能需要配置 Anthropic API Key：
```bash
# .env
